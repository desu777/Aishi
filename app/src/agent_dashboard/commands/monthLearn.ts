...
export const monthLearnCommand: Command = {
  ...
  handler: async (args, context) => {
    ...
    // Check if month-learn is available
    const availability = await checkConsolidationAvailability(context.currentUser);
    
    if (!availability.canUseMonthLearn) {
      const isTestMode = process.env.NEXT_PUBLIC_CONSOLIDATION_TEST === 'true';
      const testModeInfo = isTestMode ? '' : ' Use NEXT_PUBLIC_CONSOLIDATION_TEST=true for testing.';
      
      return {
        success: false,
        output: 'Monthly consciousness upgrade pending. Agent requires more experiential data for subconscious enhancement.',
        type: 'error'
      };
    }
